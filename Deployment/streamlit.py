    ...
    if gu_image is not None:
        ...
        try:
            ...
            if len(prediction[0]) == len(class_names):
                ...
                # Menampilkan kegunaan sebagai daftar vertikal
                kegunaan_text = ulos_info.get('Kegunaan', 'Deskripsi kegunaan belum tersedia.')
                kegunaan_list = kegunaan_text.split('\n')  # Pisahkan berdasarkan baris baru untuk membuat poin-poin

                # Tampilkan kegunaan dalam format daftar
                kegunaan_html = "<ul>"
                for item in kegunaan_list:
                    kegunaan_html += f"<li>{item}</li>"
                kegunaan_html += "</ul>"

                # Render HTML untuk kegunaan
                st.markdown(kegunaan_html, unsafe_allow_html=True)
